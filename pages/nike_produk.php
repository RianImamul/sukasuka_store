...
<div class="container mt-5">
  <div class="bg-white p-2 text-center">
    <h5 class="fw-bold">NIKE</h5>
  </div>
  <div class="row g-3 mt-3">
    <?php
    $query = $conn->query("SELECT * FROM produk_sepatu WHERE merek = 'Nike'");
    while ($row = $query->fetch_assoc()):
    ?>
    <div class="col-lg-2 col-md-3 col-sm-4 col-6">
      <div class="card h-100 text-center">
        <img src="../assets/<?= strtolower($row['merek']) ?>/<?= $row['gambar'] ?>" class="card-img-top" alt="<?= $row['nama_produk'] ?>">
        <div class="card-body d-flex flex-column">
          <h6 class="card-title"><?= htmlspecialchars($row['nama_produk']) ?></h6>
          <p class="mb-1 text-muted"><?= htmlspecialchars($row['ukuran']) ?> | <?= htmlspecialchars($row['warna']) ?></p>
          <p class="card-text">Rp. <?= number_format($row['harga'], 0, ',', '.') ?></p>
          <button class="btn btn-primary mt-auto" onclick="tambahKeKeranjang('<?= $row['nama_produk'] ?>', <?= $row['harga'] ?>, '../assets/<?= strtolower($row['merek']) ?>/<?= $row['gambar'] ?>')">Beli</button>
